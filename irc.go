...
func ircClient() {
	...
	if *f_messages != "" {
		...
		if err != nil {
			log.Fatalf("Unable to read file %v", *f_messages)
		}
	...
	client.AddCallback("001", func(event *irc.Event) {
		...
		go func(event *irc.Event) {
			...
			for i := 0; i < len(userChannels); i++ {
				t.Tick()
				debugf("[nick %v] joining channel %v on host %v", client.GetNick(), userChannels[i], host)
			...
			if *f_markov {
				...
				for {
					...
					if pair.isWaiting {
						...
						if len(channelUsers[channel]) > 0 {
							...
							// ping user
							debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), channel, nick)
				...
				for {
					...
					debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), to, message)
	...
	client.AddCallback("433", func(event *irc.Event) {
		...
		debugf("[nick %v] Switching nick to %v", client.GetNick(), newNick)
		client.Nick(newNick)
	})
	...
	client.AddCallback("JOIN", func(event *irc.Event) {
		if event.Nick == client.GetNick() {
			...
			debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), event.Arguments[0], greeting)
	...
	client.AddCallback("PRIVMSG", func(event *irc.Event) {
		if strings.HasPrefix(event.Arguments[0], "#") {
			...
			debugf("[nick %v] Received PRIVMSG in channel %v from %v: %v", client.GetNick(), channel, event.Nick, message)
			...
			if pair.isWaiting && (pair.nick == event.Nick) {
				if !pair.isPaired {
					...
					if strings.Contains(message, greeting) {
						...
						debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), channel, message)
				...
				debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), channel, greeting)
				client.Privmsg(channel, greeting)
				pair.isWaiting = true
			}
		} else {
			// private message, don't consider as a pair/conversation
			debugf("[nick %v] Received PRIVMSG from %v: %v", client.GetNick(), event.Nick, event.Message())

			if strings.Contains(event.Message(), client.GetNick()) {
				// reply to highlight with greeting
				debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), event.Nick, greeting)
				client.Privmsg(event.Nick, greeting)
			} else {
				// otherwise, create a new message
				if *f_markov {
					message := chain.Generate()
					debugf("[nick %v] Sending PRIVMSG to %v: %v", client.GetNick(), event.Nick, message)
	...
	// connect
	debugf("[nick %v] connecting to irc host %v from %v", client.GetNick(), host, original)
	for {
...
func ircServer() {
	...
	if err != nil {
		log.Fatalf("Cannot listen on %s: %v", settings.Bind, err)
	}
	debugf("Server listening on %v", settings.Bind)

	go func(sock net.Listener, events chan goircd.ClientEvent) {
		for {
			conn, err := sock.Accept()
			if err != nil {
				log.Fatalf("Error during accepting connection %v", err)
				continue
			}
